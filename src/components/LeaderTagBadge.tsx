...
export const LEADER_TAGS: Record<string, LeaderTag> = {
  ...
  educacion: { id: 'educacion', name: 'Educación', color: 'purple', icon: 'BookOpen' },
  ...
  alimentacion: { id: 'alimentacion', name: 'Alimentación', color: 'yellow', icon: 'ForkKnife' },
};
...
export default function LeaderTagBadge({
  tagId,
  size = 'sm',
  showIcon = true,
}: LeaderTagBadgeProps) {
...
export function LeaderTagsList({ tags, size = 'sm', maxVisible = 3 }: LeaderTagsListProps) {
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      ...
      {remainingCount > 0 && (
        <span
          className={`inline-flex items-center rounded-full bg-dracula-current/50 text-dracula-comment ${
            size === 'sm'
              ? 'px-2 py-0.5 text-xs'
              : size === 'md'
                ? 'px-3 py-1 text-sm'
